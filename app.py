# app.py — Panel de Señales Trading Bot 2025 (con actualización manual de gráficos)
...
def fetch_yf_clean(ticker: str, interval: str, period: str):
    ...
    # Si viene MultiIndex (algunas versiones), aplanar
...
with tabs[0]:
    ...
    if sent.empty:
        ...
        st.dataframe(sent.sort_values("FechaISO", ascending=False), use_container_width=True)
...
with tabs[1]:
    ...
    if disc.empty:
        ...
        st.dataframe(disc.sort_values("FechaISO", ascending=False), use_container_width=True)
...
with tabs[2]:
    ...
    if df.empty:
        ...
        if today_df.empty:
            ...
            st.dataframe(today_df.sort_values("HoraLocal", ascending=False), use_container_width=True)
...
with tabs[3]:
    ...
    if df.empty:
        ...
        st.dataframe(df.sort_values("FechaISO", ascending=False), use_container_width=True)
...
with tabs[5]:
    ...
    if df.empty:
        ...
        st.dataframe(df.sort_values("FechaISO", ascending=False).head(10), use_container_width=True)
...
# 8) Gráfico Avanzado (Velas + Indicadores) — manual refresh
with tabs[7]:
